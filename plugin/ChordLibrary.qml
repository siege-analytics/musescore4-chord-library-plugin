...
MuseScore {
    ...
    function generateDiagramFile(voicing) {
        ...
        // Resolve path to paste_diagram.py
        // Plugin is at .../Plugins/chordlibrary/, scripts at repo or alongside
        var pluginDir = Qt.resolvedUrl(".").toString().replace("file://", "").replace(/\/$/, "")
        var scriptPath = pluginDir + "/scripts/paste_diagram.py"
        var dataPath = pluginDir + "/voicings-cache.json"
        var reqFile = pluginDir + "/paste-request.json"

        // Write a .command shell script that runs the Python automation
        var cmd = '#!/bin/bash\n'
            + '# Auto-generated by Chord Library plugin\n'
            + 'python3 "' + scriptPath + '" '
            + '--request "' + reqFile + '" '
            + '--data "' + dataPath + '"\n'
            + 'exit 0\n'

        var cmdPath = Qt.resolvedUrl("paste-diagram.command")
        tempDiagramFile.source = cmdPath
        try {
            tempDiagramFile.write(cmd)
        } catch (e) {
            statusMsg.text = "Failed to write command: " + e
            statusMsg.color = "#c00"
            return
        }

        // Make it executable and run it
        // Qt.openUrlExternally on a .command file executes it in Terminal
