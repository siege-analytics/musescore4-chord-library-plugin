...
def make_eid() -> str:
    """Generate a MuseScore-style eid (base64-encoded UUID-like string)."""
...
def generate_fret_diagram_xml(voicing: dict, target_root: str = "C") -> str:
    ...
    fret_offset = transposed_fret - 1  # MS4 uses 0-indexed fretOffset

    lines = ["          <FretDiagram>"]

    # Properties (only write non-defaults)
    if fret_offset > 0:
        lines.append(f"            <fretOffset>{fret_offset}</fretOffset>")
    if num_frets != 4:  # 4 is default
        lines.append(f"            <frets>{num_frets}</frets>")
    if num_strings != 6:  # 6 is default
        lines.append(f"            <strings>{num_strings}</strings>")

    # Required: eid
    lines.append(f"            <eid>{make_eid()}</eid>")

    # The diagram data block
...
def generate_mscx(voicing: dict, target_root: str = "C") -> str:
    ...
    # Generate unique eids for each element
    score_eid = make_eid()
    staff_eid = make_eid()
    vbox_eid = make_eid()
    text_eid = make_eid()
    measure_eid = make_eid()
    chord_eid = make_eid()
    note_eid = make_eid()
    timesig_eid = make_eid()
